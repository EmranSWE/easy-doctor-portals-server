...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.arothiy.mongodb.net/?retryWrites=true&w=majority`;
...
function sendAppointmentEmail(booking){
    const {patient, name, treatment, date, slot}= booking;
    const email ={
...
async function run() {
    try {
        ...
        const bookingCollection = client.db('doctor_booking').collection('booking');
        const userCollection = client.db('doctor_booking').collection('users');
        const doctorCollection = client.db('doctor_booking').collection('doctors');
        const paymentCollection = client.db('doctor_booking').collection('payments');
        ...
        * app.get('/booking') //Get all booking or more then one or query
        ...
        */
        ...
        app.get('/available', async (req, res) => {
            ...
            //Step 3: for each service, find booking that service
